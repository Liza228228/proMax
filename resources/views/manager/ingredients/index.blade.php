        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border-2 border-rose-200">
                <div class="p-6 text-gray-900">
                    <!-- Форма поиска -->
                    <form method="GET" action="{{ route('manager.ingredients.index') }}" class="mb-6 bg-gradient-to-br from-rose-50 to-pink-50 p-6 rounded-xl border-2 border-rose-200">
                    ...
                    <div class="mb-6 flex justify-between items-center">
                        <div class="text-sm font-semibold text-rose-700">
                            Найдено: {{ $ingredients->total() }} ингредиентов
                        </div>
                        <a href="{{ route('manager.ingredients.create') }}"
                           class="inline-flex items-center gap-2 bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 text-white font-bold px-6 py-2.5 rounded-xl shadow-md">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-rose-200">
                            ...
                            <tbody class="bg-white divide-y divide-rose-200">
                                @forelse ($ingredients as $ingredient)
                                    <tr class="hover:bg-rose-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <form action="{{ route('manager.ingredients.destroy', $ingredient) }}" method="POST" class="inline" onsubmit="return confirm('Вы уверены, что хотите удалить этот ингредиент?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="2" class="px-6 py-4 text-center text-sm text-rose-600 font-semibold">
                    ...
                    @if($ingredients->hasPages())
                        <div class="mt-6 flex items-center justify-between">
                            <div class="text-sm text-gray-700">
                                Показано {{ $ingredients->firstItem() ?? 0 }} - {{ $ingredients->lastItem() ?? 0 }} из {{ $ingredients->total() }} ингредиентов
                            </div>
